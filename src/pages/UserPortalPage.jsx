    ...
    return (
      <div className="min-h-screen bg-[#05070A] text-slate-100 flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          ...
          <Link
            to="/login"
            className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs uppercase tracking-wider transition"
          >
            Sign In
          </Link>
        </div>
      ...
      <div className="max-w-5xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 sm:p-8 rounded-2xl bg-[#0B1120]/80 border border-slate-800 backdrop-blur-xl">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 p-0.5 shadow-lg shadow-blue-500/10">
              {user.avatarUrl ? (
                <img
                  src={user.avatarUrl}
                  alt={user.name}
                ...
                <div className="w-full h-full rounded-2xl bg-[#070B14] flex items-center justify-center text-blue-400 font-bold text-xl">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-300 text-[10px] font-mono font-semibold uppercase">
                  {user.role}
                </span>
              </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {isAdmin && (
              <Link
                to="/admin"
                className="flex-1 sm:flex-none px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs uppercase tracking-wider transition flex items-center justify-center gap-1.5 shadow-md shadow-blue-500/10"
              >
                <Shield size={14} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 rounded-2xl bg-[#0B1120]/70 border border-slate-800 space-y-3">
            ...
            <p className="text-xs text-slate-400 font-mono capitalize">
              Authenticated via <span className="text-blue-400 font-bold">{user.provider || 'LOCAL'}</span>
            </p>
          </div>
        ...
        <div className="p-8 rounded-2xl bg-[#0B1120]/80 border border-slate-800 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <Link
              to="/contact"
              className="p-5 rounded-xl bg-slate-900/60 hover:bg-slate-900 border border-slate-800 hover:border-blue-500/40 transition group"
            >
              <h4 className="text-xs font-bold text-slate-200 group-hover:text-blue-300 transition flex items-center justify-between">
                <span>Request Assessment</span>
                <ExternalLink size={13} className="text-slate-500 group-hover:text-blue-300" />
              </h4>
